...
from database import get_cached_analysis, get_today_entries, get_user, hash_image, save_analysis, set_user_vip
...
if session_id:
    try:
        ...
        if session.payment_status == "paid":
            ...
            if email:
                set_user_vip(email)
                st.session_state["verified_vip_email"] = email
                st.toast("🌟 ¡Pago confirmado! Tu cuenta VIP ya está activa.", icon="✅")
        st.query_params.clear()
    except Exception as e:
        st.error(f"Error al verificar pago: {e}")
        st.query_params.clear()

# ── VIP activation banner (legacy support) ────────────────────────────────────
elif st.query_params.get("vip_activated") == "1":
    st.toast("🌟 ¡Pago confirmado! Tu cuenta VIP ya está activa.", icon="✅")
    st.query_params.clear()
